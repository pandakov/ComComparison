...
data["name_1"] = data[["name_1", "name_1_clear"]].apply(
    lambda x: re.sub(r"[^\w\s]", " ", x["name_1_clear"].lower()).strip()
    if x.name_1 == ""
    else x["name_1"],
    axis=1,
)
data["name_2"] = data[["name_2", "name_2_clear"]].apply(
    lambda x: re.sub(r"[^\w\s]", " ", x["name_2_clear"].lower()).strip()
    if x.name_2 == ""
    else x["name_2"],
    axis=1,
)
...
word_2_vec_model = gensim.models.Word2Vec(
    [*name_1_sentences.to_list(), *name_2_sentences.to_list()],
    min_count=1,
    vector_size=150,
    window=5,
    epochs=10,
...
X_1 = [
    sent_vector(sent, vocab_w2v, word_2_vec_model)
    for sent in name_1_sentences.to_list()
]
X_2 = [
    sent_vector(sent, vocab_w2v, word_2_vec_model)
    for sent in name_2_sentences.to_list()
]

embedded_df = pd.DataFrame(np.vstack([np.array(X_1), np.array(X_2)]))
embedded_df["names"] = pd.concat(
    [data[c + "_clear"] for c in companies_columns], ignore_index=True
)
